lookupDictDefinition = function() {
    ...
    for(i = 0; i < definitions.length; i++) {
      if(i == 0
          ...
          || (definitions[i].pos != definitions[i - 1].pos)) {
              ...
              partsOfSpeech[definitions[i].pos] + "):<ol class='dict_list'>";
      }
  ...
  function fetchWords(words, selection) {
    ...
    httpRequest.open("GET", "http://localhost/dict/word_lookup.php?word=" + words, true);
    httpRequest.send();
  }
  ...
  return function() {
    ...
    if(selection.rangeCount == 0){
      //console.log("nothing selected");
        ...
        if(words == "") {
          //console.log("nothing selected 2");
        } else {
          //console.log("seletion: '" + words + "'");
